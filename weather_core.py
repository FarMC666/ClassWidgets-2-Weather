...
def _as_number(value: Any, field: str) -> float:
    ...
    try:
        ...
        raise WeatherDataError(f"天气数据缺少有效字段：{field}") from error


def _rounded_temperature(value: Any, field: str) -> int:
...
def parse_current(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(condition, dict) and temperature is not None:
        text = str(condition.get("text") or "未知天气")
        code = str(condition.get("code") or "999")
        value = _rounded_temperature(temperature, "temperature.value")
    else:
        now = payload.get("now")
        if not isinstance(now, dict):
            raise WeatherDataError("实时天气响应格式无效")
        text = str(now.get("text") or "未知天气")
        code = str(now.get("icon") or "999")
        value = _rounded_temperature(now.get("temp"), "now.temp")
    return {
        "temperature": value,
        "conditionText": text,
        "conditionCode": code,
...
def parse_daily(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(days, list) or not days or not isinstance(days[0], dict):
        raise WeatherDataError("每日预报响应中没有当天数据")
    day = days[0]
    max_value = day.get("temperatureMax", day.get("tempMax"))
    min_value = day.get("temperatureMin", day.get("tempMin"))
    return {
        "temperatureMax": _rounded_temperature(max_value, "temperatureMax.value"),
        "temperatureMin": _rounded_temperature(min_value, "temperatureMin.value"),
        "sources": _attributions(payload),
    }
...
def _event_name(alert: dict[str, Any]) -> str:
    ...
    if isinstance(event_type, dict):
        return str(event_type.get("name") or "天气")
    return str(alert.get("typeName") or alert.get("type") or "天气")
...
def _display_alert_name(event_name: str, color_code: str) -> str:
    base = event_name.strip()
    if base.endswith("预警"):
        base = base[:-2]
    color = COLOR_ZH.get(color_code, "")
    return f"{base}{color}预警"
...
def select_alert(
    ...
    event_name = _event_name(selected)
    color_code = _color_code(selected)
    return {
        "id": str(selected.get("id") or ""),
        "name": _display_alert_name(event_name, color_code),
...
def build_snapshot(
    ...
    current = parse_current(current_payload)
    daily = parse_daily(daily_payload)
    alert = select_alert(alert_payload, fetched_at)
